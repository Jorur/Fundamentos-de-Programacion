...
void vocal_inicial(char V[], int n) {						
	...
	if (V[0] == 'a' || V[0] == 'e' || V[0] == 'i' || V[0] == 'o' || V[0] == 'u') {				//if añadido para evaluar la palabra inicial
		...
		for (int i = 0; i < strlen(V) && V[i] != ' '; i++) {									//for añadido para copiar los elementos del char en un nuevo char
	...
	for (int i = 0; i < n; i++) {
		...
		while (V[i] == ' ') {								// cambio de != por ==
			...
			if (V[i + 1] == 'a' || V[i + 1] == 'e' || V[i + 1] == 'i' || V[i + 1] == 'o' || V[i + 1] == 'u') {
				...
				for (int k = i + 1; k < strlen(V) && V[k] != ' '; k++) {						//for para copiar los elementos del char en el nuevo char				
